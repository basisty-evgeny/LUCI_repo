...
public class CommonUtil {
	...
	public static String getRunningAppPkg() {
		try {
			ActivityManager am = (ActivityManager) LuciApp.getContext().getSystemService(Context.ACTIVITY_SERVICE);
	...
	public static void launchMarket() {
		Uri uri = Uri.parse("market://details?id=" + LuciApp.getContext().getPackageName());
		Intent goToMarket = new Intent(Intent.ACTION_VIEW, uri);
		try {
			LuciApp.getContext().startActivity(goToMarket);

		} catch (Exception e) {
			// open with browser
			Intent browserIntent = new Intent(Intent.ACTION_VIEW,
					Uri.parse("https://play.google.com/store/apps/details?id=" + LuciApp.getContext().getPackageName()));
			browserIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
			LuciApp.getContext().startActivity(browserIntent);
		}
	}
    ...
    public static void changeLanguage(int index) {
        Resources res = LuciApp.getContext().getResources();
